/**
 * rede.tsx — migrado para Firestore
 * Lê usuários da coleção "users" em tempo real via onSnapshot.
 * Remove completamente o AsyncStorage.getItem("@App:users").
 */
...
function UserAvatar({ user, size = 42 }: { user: StoredUser; size?: number }) {
  if (user.photo) {
    return (
      <Image source={{ uri: user.photo }} style={{ width: size, height: size, borderRadius: size / 2 }} />
    );
  }
  return (
...
function UserCard({
  user, badge, rank, followersCount,
}: {
  user: StoredUser; badge: "followed" | "liked" | "disliked" | "none";
  rank?: number; followersCount: number;
}) {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.8}
      onPress={() => router.push({ pathname: "/perfil", params: { viewUserEmail: user.email } })}
    >
      {rank !== undefined && (
        <View style={[
          styles.rankBadge,
          isFeaturedFollowed ? styles.rankBadgeFollowed
            : isFeaturedLiked ? styles.rankBadgeLiked
...
export default function RedeScreen() {
  ...
  // ── Listener em tempo real na coleção "users" ──────────────────────────────
  useEffect(() => {
    const unsub = onSnapshot(
      collection(db, "users"),
      (snapshot) => {
        const clean: StoredUser[] = snapshot.docs.map((d) => {
  ...
  const { listData, hiddenCount } = useMemo(() => {
    ...
    if (q) {
      ...
      const items: ListItem[] = visible.map(({ user, fCount }) => ({
        type: "user", user, badge: "none", followersCount: fCount,
      }));
    ...
    const topLiked    = byLikes.filter(({ user }) => (user.likes?.length ?? 0) > 0).slice(0, TOP_LIKED_COUNT);
    ...
    const topDisliked    = byDislikes.filter(({ user }) => (user.dislikes?.length ?? 0) > 0).slice(0, TOP_DISLIKED_COUNT);
    const topDislikedIds = new Set(topDisliked.map(({ user }) => user.email));

    const remaining        = withFollowers.filter(({ user }) =>
      !topFollowedIds.has(user.email) && !topLikedIds.has(user.email) && !topDislikedIds.has(user.email)
    );
    ...
    if (remaining.length > 0) {
      ...
      if (visibleCount < remaining.length) items.push({ type: "show-more" });
    }

    return { listData: items, hiddenCount: Math.max(0, remaining.length - visibleCount) };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        <TextInput
          style={styles.searchInput} value={search} onChangeText={handleSearch}
          placeholder="Buscar usuários..." placeholderTextColor="#CCC"
      ...
      <FlatList
        ...
        keyExtractor={(item, idx) =>
          item.type === "user" ? `user-${item.user.email}` : `${item.type}-${idx}`
        }
        contentContainerStyle={styles.listContent}
        renderItem={({ item }) => {
          if (item.type === "top-followed-label") return (
            <View style={styles.sectionLabel}>
              <MaterialIcons name="star" size={15} color="#FFD700" />
              <Text style={[styles.sectionLabelText, styles.sectionLabelFollowed]}>Mais seguidos</Text>
            </View>
          );
          if (item.type === "top-liked-label") return (
            <View style={styles.sectionLabel}>
              <MaterialIcons name="thumb-up" size={15} color="#4169E1" />
              <Text style={styles.sectionLabelText}>Mais curtidos</Text>
            </View>
          );
          if (item.type === "top-disliked-label") return (
            <View style={styles.sectionLabel}>
              <MaterialIcons name="thumb-down" size={15} color="#E63946" />
              <Text style={[styles.sectionLabelText, styles.sectionLabelDisliked]}>Mais rejeitados</Text>
            </View>
          );
          if (item.type === "all-label") return (
            <View style={styles.sectionLabel}>
              <MaterialIcons name="people" size={15} color="#999" />
              <Text style={[styles.sectionLabelText, styles.sectionLabelMuted]}>Todos os usuários</Text>
            </View>
          );
          if (item.type === "user") return (
            <UserCard user={item.user} badge={item.badge} rank={getRank(item)} followersCount={item.followersCount} />
          );
          if (item.type === "show-more") {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => setVisibleCount((v) => v + PAGE_SIZE)} activeOpacity={0.8}
              >
        ...
        ListEmptyComponent={isEmpty ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>
              {search.trim() ? "Tente buscar por outro nome ou e-mail" : "Nenhum usuário cadastrado ainda"}
            </Text>
          </View>
        ) : null}
      />
...
const styles = StyleSheet.create({
  ...
  backButton: { width: 40, height: 40, borderRadius: 12, backgroundColor: "#FFF", justifyContent: "center", alignItems: "center", elevation: 2 },
  headerTitle: { fontSize: 24, fontWeight: "800", color: "#212529" },
  searchContainer: { flexDirection: "row", alignItems: "center", backgroundColor: "#FFF", borderRadius: 14, marginHorizontal: 20, marginBottom: 16, paddingHorizontal: 14, elevation: 2, height: 48 },
  searchIcon: { marginRight: 8 },
  searchInput: { flex: 1, fontSize: 15, color: "#333", paddingVertical: 0 },
  clearButton: { padding: 4 },
  listContent: { paddingHorizontal: 20, paddingBottom: 30 },
  sectionLabel: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 12, marginTop: 4, paddingHorizontal: 2 },
  sectionLabelText: { fontSize: 12, fontWeight: "800", color: "#4169E1", textTransform: "uppercase", letterSpacing: 0.6 },
  sectionLabelFollowed: { color: "#B8860B" },
  sectionLabelDisliked: { color: "#E63946" },
  sectionLabelMuted: { color: "#999" },
  card: { backgroundColor: "#FFF", borderRadius: 20, padding: 16, marginBottom: 12, flexDirection: "row", alignItems: "center", elevation: 3 },
  cardFollowed: { backgroundColor: "#FFFDF0", borderWidth: 1.5, borderColor: "#FFD700", elevation: 5 },
  cardLiked: { backgroundColor: "#EEF4FF", borderWidth: 1, borderColor: "#C7D2FE", elevation: 4 },
  cardDisliked: { backgroundColor: "#FFF5F5", borderWidth: 1, borderColor: "#FECACA", elevation: 4 },
  rankBadge: { position: "absolute", top: 10, right: 20, borderRadius: 8, paddingHorizontal: 6, paddingVertical: 2 },
  ...
  cardIcon: { width: 50, height: 50, borderRadius: 25, backgroundColor: "#EEF2FF", justifyContent: "center", alignItems: "center", marginRight: 14, overflow: "hidden" },
  ...
  followersRow: { flexDirection: "row", alignItems: "center", gap: 4, marginTop: 4 },
  followersText: { fontSize: 12, color: "#4169E1", fontWeight: "600" },
  cardMeta: { flexDirection: "row", alignItems: "center", gap: 4, marginRight: 6, marginTop: 30 },
  cardMetaText: { fontSize: 13, color: "#999" },
  cardMetaDot: { fontSize: 13, color: "#CCC", marginHorizontal: 2 },
  avatarFallback: { backgroundColor: "#4169E1", justifyContent: "center", alignItems: "center" },
  avatarFallbackText: { color: "#FFF", fontWeight: "700" },
  showMoreButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 14, marginTop: 4, marginBottom: 10, backgroundColor: "#EEF2FF", borderRadius: 14, borderWidth: 1, borderColor: "#C7D2FE" },
