/**
 * perfil.tsx — migrado para Firestore
 *
 * Única mudança em relação ao original: a função `loadTarget` dentro de
 * `OtherUserProfile` deixa de ler `@App:users` no AsyncStorage e passa a
 * consultar o documento `users/{email}` direto no Firestore.
 *
 * Todo o restante do arquivo (ProfileGallery, UserCommentsCard,
 * FollowStatsCard, ProfileScreen próprio) é mantido idêntico.
 */
...
// ── Firestore ──────────────────────────────────────────────────────────────────
import { db, doc, getDoc } from "../src/services/firebaseConfig";
...
function PhotoSlideViewer({
  photos, initialIndex, currentUserEmail, onClose, onToggleLike,
  onDelete, timeLeft, ownerName, ownerPhoto,
}: SlideViewerProps) {
    ...
    <View style={slideStyles.root}>
      ...
      {photos.length > 1 && (
        <View style={slideStyles.counter}>
          <Text style={slideStyles.counterText}>{currentIndex + 1} / {photos.length}</Text>
        </View>
      )}
      <FlatList
        ref={flatRef} data={photos} keyExtractor={(p) => p.id}
        horizontal pagingEnabled showsHorizontalScrollIndicator={false}
        initialScrollIndex={initialIndex}
        getItemLayout={(_, index) => ({ length: screenWidth, offset: screenWidth * index, index })}
        onViewableItemsChanged={onViewable} viewabilityConfig={viewabilityConfig}
        renderItem={({ item }) => (
          <View style={[slideStyles.slide, { width: screenWidth }]}>
            <Image source={{ uri: item.imageUri }} style={slideStyles.slideImage} resizeMode="contain" />
          </View>
        )}
      />
      {photos.length > 1 && (
        <View style={slideStyles.dots}>
          {photos.map((_, i) => (
            <View key={i} style={[slideStyles.dot, i === currentIndex && slideStyles.dotActive]} />
          ))}
        </View>
      )}
      <View style={slideStyles.footer}>
        <View style={slideStyles.userRow}>
          {(ownerPhoto ?? current?.userPhoto) ? (
            <Image source={{ uri: ownerPhoto ?? current?.userPhoto }} style={slideStyles.avatar} />
          ) : (
        ...
        <View style={slideStyles.actions}>
          {isOwn && onDelete ? (
            <TouchableOpacity style={slideStyles.actionBtn} onPress={() => {
              onDelete(current.id);
            ...
            currentUserEmail && (
              <TouchableOpacity style={slideStyles.actionBtn} onPress={() => onToggleLike(current.id)}>
                <MaterialIcons name={liked ? "favorite" : "favorite-border"} size={28} color={liked ? "#FF6B6B" : "#FFF"} />
...
const slideStyles = StyleSheet.create({
  ...
  counter: { position: "absolute", top: 54, left: 20, zIndex: 20, backgroundColor: "rgba(0,0,0,0.4)", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  counterText: { color: "#FFF", fontSize: 13, fontWeight: "700" },
  slide: { flex: 1, justifyContent: "center", alignItems: "center" },
  slideImage: { width: "100%", height: "100%" },
  dots: { position: "absolute", bottom: 120, left: 0, right: 0, flexDirection: "row", justifyContent: "center", gap: 6, zIndex: 10 },
  dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: "rgba(255,255,255,0.4)" },
  dotActive: { backgroundColor: "#FFF", width: 18 },
  footer: { position: "absolute", bottom: 0, left: 0, right: 0, flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 24, paddingVertical: 24, paddingBottom: 44, backgroundColor: "rgba(0,0,0,0.55)", zIndex: 10 },
  userRow: { flexDirection: "row", alignItems: "center", gap: 12 },
  avatar: { width: 42, height: 42, borderRadius: 21, borderWidth: 2, borderColor: "#FFF" },
  avatarFallback: { width: 42, height: 42, borderRadius: 21, backgroundColor: "#4169E1", justifyContent: "center", alignItems: "center" },
...
// ── ProfileGallery (inalterado) ────────────────────────────────────────────────
function ProfileGallery({ userEmail, currentUserEmail }: { userEmail: string; currentUserEmail?: string }) {
  ...
  const handleDelete = useCallback((photoId: string) => { deletePhoto(photoId); }, [deletePhoto]);
    ...
    <View style={galStyles.card}>
      <View style={galStyles.cardHeader}>
        ...
        <Text style={galStyles.cardSub}>
          {photos.length === 0 ? "Nenhuma foto ativa" : `${photos.length} foto${photos.length > 1 ? "s" : ""} · 24h`}
        </Text>
      </View>
      {photos.length === 0 ? (
        ...
        <View style={galStyles.grid}>
          {photos.slice(0, 9).map((photo, index) => {
            const liked = currentUserEmail ? photo.likes.includes(currentUserEmail) : false;
            const isNinthWithMore = index === 8 && photos.length > 9;
            return (
              <TouchableOpacity key={photo.id} style={galStyles.thumb} activeOpacity={0.85} onPress={() => setSlideIndex(index)}>
                <Image source={{ uri: photo.imageUri }} style={galStyles.thumbImg} resizeMode="cover" />
      ...
      <Modal visible={slideIndex !== null} animationType="fade" statusBarTranslucent onRequestClose={() => setSlideIndex(null)}>
        {slideIndex !== null && (
          <PhotoSlideViewer
            photos={photos} initialIndex={slideIndex} currentUserEmail={currentUserEmail}
            onClose={() => setSlideIndex(null)} onToggleLike={handleToggleLike}
            onDelete={currentUserEmail === userEmail ? handleDelete : undefined} timeLeft={timeLeft}
          />
        )}
      </Modal>
    </View>
...
const galStyles = StyleSheet.create({
  card: { backgroundColor: "#FFF", borderRadius: 16, marginHorizontal: 20, marginBottom: 14, overflow: "hidden", elevation: 2 },
  cardHeader: { flexDirection: "row", alignItems: "center", gap: 8, padding: 16, borderBottomWidth: 1, borderBottomColor: "#F0F0F0" },
  ...
  thumb: { width: GALLERY_SIZE, height: GALLERY_SIZE, margin: 2, position: "relative" },
  thumbImg: { width: "100%", height: "100%" },
  thumbBadge: { position: "absolute", bottom: 4, right: 4, flexDirection: "row", alignItems: "center", gap: 2, backgroundColor: "rgba(0,0,0,0.55)", paddingHorizontal: 5, paddingVertical: 2, borderRadius: 10 },
  thumbLikes: { color: "#FFF", fontSize: 10, fontWeight: "700" },
  moreOverlay: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.52)", justifyContent: "center", alignItems: "center" },
  moreOverlayText: { color: "#FFF", fontSize: 22, fontWeight: "800", letterSpacing: 0.5 },
});

// ── UserCommentsCard (inalterado) ─────────────────────────────────────────────
interface UserCommentEntry { comment: ForumComment; room: ForumRoom; }
...
function UserCommentsCard({ userEmail }: { userEmail: string }) {
  ...
  const entries: UserCommentEntry[] = React.useMemo(() => {
    ...
    for (const room of rooms)
      for (const comment of room.comments)
        if (comment.userEmail === userEmail) result.push({ comment, room });
  ...
  return (
    <>
      <TouchableOpacity style={cmtStyles.card} activeOpacity={0.75} onPress={() => setExpanded(true)}>
        <View style={cmtStyles.iconWrap}><MaterialIcons name="forum" size={22} color="#4169E1" /></View>
        <View style={cmtStyles.textWrap}>
          <Text style={cmtStyles.title}>Comentários no Fórum</Text>
          <Text style={cmtStyles.sub}>
            {count === 0 ? "Nenhum comentário ainda"
              : `${count} comentário${count !== 1 ? "s" : ""} em ${new Set(entries.map((e) => e.room.id)).size} sala${new Set(entries.map((e) => e.room.id)).size !== 1 ? "s" : ""}`}
          </Text>
        </View>
        ...
        <View style={cmtStyles.modalRoot}>
          ...
          {count === 0 ? (
            <View style={cmtStyles.empty}>
              ...
              <Text style={cmtStyles.emptySub}>Participe de uma sala de fórum para ver seus comentários aqui.</Text>
            </View>
          ) : (
            <FlatList
              data={entries} keyExtractor={(e) => e.comment.id}
              contentContainerStyle={cmtStyles.listContent}
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={cmtStyles.commentCard} activeOpacity={0.8}
                  onPress={() => { setExpanded(false); router.push({ pathname: "/forum-room", params: { roomId: item.room.id } }); }}
                >
                  <View style={cmtStyles.roomRow}>
                    <MaterialIcons name="forum" size={13} color="#4169E1" />
                    <Text style={cmtStyles.roomTitle} numberOfLines={1}>{item.room.articleTitle}</Text>
                  </View>
                  <Text style={cmtStyles.commentText} numberOfLines={4}>{item.comment.text}</Text>
                  <View style={cmtStyles.commentFooter}>
                    <Text style={cmtStyles.commentDate}>
                      ...
                      {new Date(item.comment.createdAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                    </Text>
...
const cmtStyles = StyleSheet.create({
  card: { flexDirection: "row", alignItems: "center", backgroundColor: "#FFF", borderRadius: 12, padding: 16, elevation: 1, gap: 14 },
  ...
  modalHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, paddingTop: 60, paddingBottom: 16, backgroundColor: "#FFF", elevation: 2 },
  modalBackBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: "#F8F9FA", justifyContent: "center", alignItems: "center" },
  modalTitle: { fontSize: 18, fontWeight: "800", color: "#212529" },
  listContent: { padding: 16, gap: 12 },
  commentCard: { backgroundColor: "#FFF", borderRadius: 16, padding: 14, elevation: 2, borderLeftWidth: 3, borderLeftColor: "#4169E1" },
...
// ── FollowStatsCard (inalterado) ───────────────────────────────────────────────
function FollowStatsCard({ targetEmail, currentUserEmail }: { targetEmail: string; currentUserEmail?: string }) {
  ...
  return (
    <>
      <View style={followStatsStyles.card}>
        <TouchableOpacity style={followStatsStyles.stat} activeOpacity={0.75} onPress={() => setModal("followers")}>
        ...
        <TouchableOpacity style={followStatsStyles.stat} activeOpacity={0.75} onPress={() => setModal("following")}>
          <Text style={followStatsStyles.statNum}>{following.length}</Text>
          <Text style={followStatsStyles.statLabel}>Seguindo</Text>
        </TouchableOpacity>
      </View>
      {modal && <FollowersModal visible={!!modal} onClose={() => setModal(null)} targetEmail={targetEmail} mode={modal} />}
    </>
  );
}

const followStatsStyles = StyleSheet.create({
  card: { flexDirection: "row", backgroundColor: "#FFF", borderRadius: 16, marginHorizontal: 20, marginBottom: 14, elevation: 2, overflow: "hidden" },
...
// ── OtherUserProfile — MIGRADO PARA FIRESTORE ──────────────────────────────────
function OtherUserProfile({ targetEmail }: { targetEmail: string }) {
  ...
  // ── Lê o perfil do usuário alvo diretamente do Firestore ──────────────────
  const loadTarget = async () => {
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        const { passwordHash, ...pub } = data;
        setTarget({
          name:       pub.name       ?? "",
          email:      pub.email      ?? targetEmail,
          photo:      pub.photo      ?? undefined,
          profession: pub.profession ?? undefined,
          likes:      Array.isArray(pub.likes)    ? pub.likes    : [],
          dislikes:   Array.isArray(pub.dislikes) ? pub.dislikes : [],
        });
      }
    } catch (e) {
      console.error("Erro ao carregar usuário (Firestore):", e);
    } finally {
  ...
  const handleReact = async (type: "like" | "dislike") => {
    ...
    await loadTarget(); // recarrega do Firestore para refletir o novo estado
  };
  ...
  const handleFollow = async () => {
    ...
    if (nowFollowing) {
      await sendNotification({ type: "profile_like", fromName: loggedUser.name });
    }
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.profileSection}>
        <View style={styles.avatarWrapper}>
          <StoryRing userEmail={target.email} userName={target.name} userPhoto={target.photo} size={90} currentUserEmail={loggedUser?.email} />
        </View>
        ...
        {!isSelf && loggedUser && (
          <Animated.View style={{ transform: [{ scale: starScale }], marginBottom: 6 }}>
            ...
            >
              <MaterialIcons name={following ? "star" : "star-outline"} size={20} color={following ? "#FFD700" : "#4169E1"} />
        ...
        <View style={styles.reactionContainer}>
          <TouchableOpacity style={styles.reactionButton} onPress={() => handleReact("dislike")} disabled={isSelf || !loggedUser}>
            <MaterialIcons name="thumb-down" size={26} color={dislikedByMe ? "#E63946" : "#CCC"} />
          </TouchableOpacity>
          <Text style={styles.reactionCount}>{target.dislikes?.length ?? 0}</Text>
          <TouchableOpacity style={styles.reactionButton} onPress={() => handleReact("like")} disabled={isSelf || !loggedUser}>
            <MaterialIcons name="thumb-up" size={26} color={likedByMe ? "#4169E1" : "#CCC"} />
          </TouchableOpacity>
      ...
      <View style={styles.infoSection}><UserCommentsCard userEmail={target.email} /></View>
...
// ── ProfileScreen (próprio usuário — inalterado) ───────────────────────────────
export default function ProfileScreen() {
  ...
  const openEditModal = () => {
    setEditName(user?.name ?? ""); setEditPhotoUrl(user?.photo ?? "");
    setEditProfession(user?.profession ?? ""); setPhotoPreviewError(false);
  ...
  const pickFromGallery = async () => {
    ...
    if (!result.canceled) { setEditPhotoUrl(result.assets[0].uri); setPhotoPreviewError(false); }
  };
  ...
  const handleSaveProfile = async () => {
    if (!editName.trim() || editName.trim().length < 2) { Alert.alert("Nome inválido", "O nome deve ter no mínimo 2 caracteres."); return; }
    setIsSaving(true);
    try {
  ...
  const handleLogout = () => {
    Alert.alert("Sair da Conta", "Tem certeza que deseja sair?", [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        onPress: async () => {
          try { setIsLoggingOut(true); await signOut(); router.replace("/login"); }
          catch (error: any) { Alert.alert("Erro", error.message || "Erro ao fazer logout"); }
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView style={[styles.container, isDark && styles.darkBg]} contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          <TouchableOpacity style={styles.backButton} onPress={() => router.back()}>
        ...
        <View style={[styles.profileSection, isDark && styles.darkCard]}>
          <View style={styles.avatarWrapper}>
            <StoryRing userEmail={user.email} userName={user.name} userPhoto={user.photo} size={90} isOwn onAddPhoto={handleAddStoryPhoto} currentUserEmail={user.email} />
          </View>
          ...
          <View style={styles.reactionContainer}>
            <TouchableOpacity onPress={() => handleProfileReaction(user.email, "dislike")} style={styles.reactionButton}>
              <MaterialIcons name="thumb-down" size={24} color={(user.dislikes ?? []).includes(user.email) ? "#E63946" : "#999"} />
            </TouchableOpacity>
            <Text style={[styles.reactionCount, isDark && styles.darkText]}>{user.dislikes?.length ?? 0}</Text>
            <TouchableOpacity onPress={() => handleProfileReaction(user.email, "like")} style={styles.reactionButton}>
              <MaterialIcons name="thumb-up" size={24} color={(user.likes ?? []).includes(user.email) ? "#4169E1" : "#999"} />
            </TouchableOpacity>
            <Text style={[styles.reactionCount, isDark && styles.darkText]}>{user.likes?.length ?? 0}</Text>
          </View>
        ...
        <View style={styles.infoSection}><UserCommentsCard userEmail={user.email} /></View>
        ...
        <View style={styles.infoSection}>
          ...
          <View style={[styles.infoCard, isDark && styles.darkCard]}>
            ...
            <View style={styles.infoContent}>
              ...
              <Text style={[styles.infoValue, isDark && styles.darkText]}>{new Date().toLocaleDateString("pt-BR")}</Text>
            </View>
          </View>
        </View>

        <TouchableOpacity style={styles.logoutItem} onPress={handleLogout} disabled={isLoggingOut}>
          {isLoggingOut ? <ActivityIndicator color="#E63946" size="small" /> : <MaterialIcons name="logout" size={22} color="#E63946" />}
        ...
        {/* Modal de edição */}
        <Modal visible={editModalVisible} animationType="slide" transparent onRequestClose={() => !isSaving && setEditModalVisible(false)}>
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>Editar Perfil</Text>
                <TouchableOpacity onPress={() => !isSaving && setEditModalVisible(false)} disabled={isSaving}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.modalAvatarArea}>
                  {showPreview ? (
                    <Image source={{ uri: previewUrl }} style={styles.modalAvatarImage} onError={() => setPhotoPreviewError(true)} />
                  ) : (
                    <View style={styles.modalAvatarPlaceholder}>
                      <Text style={styles.modalAvatarInitial}>{(editName || user.name).charAt(0).toUpperCase()}</Text>
                    </View>
                  )}
                </View>
                ...
                <View style={styles.inputContainer}>
                  ...
                  <TextInput style={styles.textInput} value={editName} onChangeText={setEditName} placeholder="Seu nome" placeholderTextColor="#CCC" maxLength={50} editable={!isSaving} />
                </View>
                ...
                <View style={styles.inputContainer}>
                  ...
                  <TextInput style={styles.textInput} value={editProfession} onChangeText={setEditProfession} placeholder="Ex: Desenvolvedor, Designer, Estudante..." placeholderTextColor="#CCC" maxLength={60} editable={!isSaving} />
                </View>
                ...
                <View style={[styles.inputContainer, { alignItems: "flex-start", paddingTop: 14 }]}>
                  ...
                  <TextInput style={[styles.textInput, { minHeight: 44 }]} value={editPhotoUrl} onChangeText={(v) => { setEditPhotoUrl(v); setPhotoPreviewError(false); }} placeholder="https://exemplo.com/foto.jpg" placeholderTextColor="#CCC" autoCapitalize="none" keyboardType="url" editable={!isSaving} multiline />
                </View>
                {photoPreviewError && editPhotoUrl.trim().length > 0 && <Text style={styles.photoErrorText}>URL inválida ou imagem não carregou.</Text>}
                {editPhotoUrl.trim().length > 0 && (
                  <TouchableOpacity style={styles.removePhotoButton} onPress={() => { setEditPhotoUrl(""); setPhotoPreviewError(false); }}>
                ...
                <TouchableOpacity style={[styles.saveButton, isSaving && styles.saveButtonDisabled]} onPress={handleSaveProfile} disabled={isSaving}>
                  {isSaving ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={styles.saveButtonText}>Salvar alterações</Text>}
                </TouchableOpacity>
              </ScrollView>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  header: { paddingTop: 50, paddingHorizontal: 20, paddingBottom: 20, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  headerTitle: { fontSize: 18, fontWeight: "800", color: "#212529" },
  backButton: { padding: 8 },
  editButton: { flexDirection: "row", alignItems: "center", gap: 4, paddingVertical: 8, paddingHorizontal: 14, backgroundColor: "#EEF2FF", borderRadius: 20 },
  editButtonText: { color: "#4169E1", fontWeight: "700", fontSize: 14 },
  profileSection: { alignItems: "center", padding: 30, backgroundColor: "#FFF", marginHorizontal: 20, marginTop: 10, marginBottom: 14, borderRadius: 20, elevation: 2 },
  avatarWrapper: { marginBottom: 12 },
  followStarBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 20, paddingVertical: 9, borderRadius: 22, borderWidth: 1.5, borderColor: "#4169E1", backgroundColor: "#EEF2FF", marginBottom: 10 },
  followStarBtnActive: { backgroundColor: "#FFF9E6", borderColor: "#FFD700" },
  followStarText: { fontSize: 14, fontWeight: "700", color: "#4169E1" },
  followStarTextActive: { color: "#B8860B" },
  followersPill: { backgroundColor: "#4169E1", borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2, marginLeft: 2 },
  ...
  reactionContainer: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 16 },
  reactionButton: { padding: 6 },
  reactionCount: { fontWeight: "bold", color: "#333", fontSize: 15 },
  addStoryBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingVertical: 8, paddingHorizontal: 18, backgroundColor: "#EEF2FF", borderRadius: 20 },
  ...
  loginButton: { backgroundColor: "#4169E1", paddingHorizontal: 30, paddingVertical: 12, borderRadius: 12, marginTop: 10 },
  loginButtonText: { color: "#FFF", fontWeight: "700", fontSize: 16 },
  infoSection: { paddingHorizontal: 20, marginTop: 0, gap: 12, marginBottom: 12 },
  infoCard: { flexDirection: "row", backgroundColor: "#FFF", padding: 16, borderRadius: 12, alignItems: "center", elevation: 1 },
  ...
  logoutItem: { flexDirection: "row", alignItems: "center", padding: 20, marginHorizontal: 20, marginTop: 20, backgroundColor: "#FFF", borderRadius: 12, elevation: 1 },
  ...
  modalContainer: { backgroundColor: "#FFF", borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 28, paddingBottom: 40, maxHeight: "90%" },
  ...
  modalAvatarPlaceholder: { width: 90, height: 90, borderRadius: 45, backgroundColor: "#4169E1", justifyContent: "center", alignItems: "center" },
  modalAvatarInitial: { color: "#FFF", fontSize: 36, fontWeight: "bold" },
  galleryBtn: { flexDirection: "row", backgroundColor: "#333", padding: 12, borderRadius: 10, alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 20 },
  galleryBtnText: { color: "#FFF", fontWeight: "600", fontSize: 14 },
  inputLabel: { fontSize: 13, fontWeight: "600", color: "#666", marginBottom: 8 },
  inputContainer: { flexDirection: "row", alignItems: "center", backgroundColor: "#F8F9FA", borderRadius: 14, paddingHorizontal: 16, paddingVertical: 14, borderWidth: 1, borderColor: "#EEE", marginBottom: 16 },
  textInput: { flex: 1, fontSize: 15, color: "#333" },
  photoErrorText: { fontSize: 12, color: "#E63946", marginBottom: 10, marginLeft: 4 },
  removePhotoButton: { flexDirection: "row", alignItems: "center", gap: 6, alignSelf: "flex-start", marginBottom: 20, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 20, borderWidth: 1, borderColor: "#E63946" },
  removePhotoText: { color: "#E63946", fontSize: 13, fontWeight: "600" },
  saveButton: { backgroundColor: "#4169E1", paddingVertical: 18, borderRadius: 16, alignItems: "center", marginTop: 8 },
