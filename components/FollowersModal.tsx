/**
 * FollowersModal.tsx
 * Modal que exibe a lista de seguidores ou seguindo de um usuário.
 */
...
interface FollowersModalProps {
  ...
  onClose: () => void;
  targetEmail: string;
  /** "followers" = quem segue; "following" = quem esse usuário segue */
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem("@App:users")
      .then((raw) => {
        ...
        setAllUsers(
          parsed.map(({ passwordHash, ...u }) => u as PublicUser)
        );
      })
      .catch(() => {})
      ...
      <View style={styles.root}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              const following =
                user ? isFollowing(user.email, item.email) : false;
                ...
                <View style={styles.userRow}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      router.push({
                        pathname: "/perfil",
                        params: { viewUserEmail: item.email },
                      });
                    }}
                  >
                    {item.photo ? (
